...
function Sidebar() {
    ...
    return (
        <div className='w-[26%] h-full rounded-2xl border border-[#cfd8dc] bg-white shadow-sm'>
            <div className='relative flex flex-col h-full items-center py-4'>
                <img className='w-24 h-24' src={logo} />
                <h1 className='font-bold py-3 text-lg text-[#323232]'>Admin Dashboard</h1>
                <div className='grid grid-rows-4 w-full px-3 py-4 gap-1'>
                    {
                        items.map((item, i) => {
                            return (
                                <div key={item} onClick={() => { setIndex(i) }} className='w-full h-12 cursor-pointer'>
                                    <div className={`${i === index ? 'text-white bg-[#49a54d]' : 'text-[#607d8b]'} h-full flex flex-row items-center px-6 gap-4 rounded-lg`}>
                                        {item.icon}
