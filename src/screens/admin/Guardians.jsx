...
function Guardians({ fetchState, records, sms, setSms }) {
  ...
  const messageConstructor = (report) => {
    ...
    return (
      <p className="py-3 text-sm">
        A SMS message was sent to Mrs./Mr.{" "}
        <span className="font-roboto-bold">
          {report.student.guardian.name}{" "}
        </span>
        to give information about her/his child{" "}
        <span className="font-roboto-bold">
          {report.student.name}, {status[report.status]}{" "}
        </span>
        the school on{" "}
        <span className="font-roboto-bold">
          {format(report.dateRecord.toDate(), "EEEE, MMMM dd, yyyy")} at
          {format(report.dateRecord.toDate(), " hh:mm a")}
        </span>
        .
      </p>
    );
          ...
          <div className="w-full h-full">
            {fetchState != 1 ? (
              ...
              <div className="flex flex-col gap-2 h-[400px] overflow-auto">
                {!records[format(date, "yyyy/MM/dd")] ? (
                  <p className="py-40 text-center">No records...</p>
                ) : records[format(date, "yyyy/MM/dd")].filter(
                    (item) => item.sms && item.student.guardian
                  ).length == 0 ? (
                  <p className="py-40 text-center">No records...</p>
                ) : (
                  records[format(date, "yyyy/MM/dd")]
                      ...
                      return (
                        <div className="flex flex-col px-4 py-4 h-36 w-full shadow-sm border rounded-[10px]">
                          <div className="flex flex-row text-[22px] items-center gap-2">
                            ...
                            <h1 className="font-roboto-bold text-base">
                              Guardian Report
                            </h1>
                          </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col">
              ...
              <div className="w-full flex flex-row gap-2 items-center">
                ...
                />
                <p className="text-sm">{`SMS service ${
